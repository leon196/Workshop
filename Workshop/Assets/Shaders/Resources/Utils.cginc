...
{
	return ceil(amount * segments) / segments;
}

float2 pixelize(float2 uv, float segments)
{
	return ceil(uv * segments) / segments;
}
...
{
	return float4(ceil(color.rgb * segments) / segments, 1.0);
}
...
float4 screenVectorFrom (float angle, float radius, float aspect)
{
  return float4(cos(angle) * aspect, sin(angle), 0, 0) * radius;
}
